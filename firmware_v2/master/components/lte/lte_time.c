...
int64_t lte_utc_datetime_to_epoch_ms(int year, int month, int day, int hour, int minute,
                                        int second)
{
...
bool lte_time_parse_cclk(const char *resp, int64_t *epoch_ms_utc_out)
{
    ...
    int64_t epoch_ms = lte_utc_datetime_to_epoch_ms(yy, mo, dd, hh, mi, ss);
    if (epoch_ms < 0) {
        return false;
    }
...
int lte_format_ist(uint64_t epoch_ms_utc, char *out, size_t out_len)
{
    if (out == NULL || out_len < 20) {
        return -1;
    }
    /* IST = UTC + 5:30 */
    int64_t local_ms = (int64_t)epoch_ms_utc + (int64_t)LTE_TZ_QUARTERS_IST * 15LL * 60LL * 1000LL;
    if (local_ms < 0) {
        return -1;
    }
