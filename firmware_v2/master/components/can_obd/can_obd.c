...
esp_err_t can_obd_init(void)
{
    ...
    /* PCB pins from board_pins.h (Kconfig defaults match; prefer soldered macros). */
    esp_err_t err = mcp2515_init(BOARD_MCP_SCK_GPIO, BOARD_MCP_MOSI_GPIO, BOARD_MCP_MISO_GPIO,
                                 BOARD_MCP_CS_GPIO, SPI_CLOCK_HZ);
    s_mcp_present = (err == ESP_OK);
    return err;
}
