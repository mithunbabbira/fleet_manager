...
static fw_ota_state_t s_state = FW_OTA_STATE_IDLE;
...
esp_err_t fw_ota_begin(size_t expected_size, const char *sha256_hex)
{
    ...
    if (s_state == FW_OTA_STATE_WRITING) {
        xSemaphoreGive(s_mu);
        return ESP_ERR_INVALID_STATE;
    }
...
esp_err_t fw_ota_end_and_reboot(void)
{
    ...
    s_state = FW_OTA_STATE_PENDING_REBOOT;
...
bool fw_ota_is_busy(void)
{
    ...
    return st.state == FW_OTA_STATE_WRITING || st.state == FW_OTA_STATE_PENDING_REBOOT;
}
